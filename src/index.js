...
function BrightestStar() {
  ...
  const SANFRANCISCO = { latitude: 37.79, longitude: -122.4 };

  this.init = async function () {
    try {
      const response = await fetch(
        "https://ssd-api.jpl.nasa.gov/fireball.api?date-min=2017-01-01&req-loc=true&limit=4"
      );
      const list = this.mapToSignedCoordinateList(await response.json());
      const brightestStar = this.filterListFindBrightestStar(list);
      if (brightestStar)
        console.log(
          "Nasa Nasa on the Earth who's the Brightest Star of them all!!! \n\n Star with latitude",
          brightestStar.lat,
          " and longitude ",
          brightestStar.lon,
          " is the brightest of them all!!! \n Energy that is high as ",
          brightestStar.energy,
          " joules"
        );
    } catch (ex) {
      console.error(ex);
  ...
  this.mapToSignedCoordinateList = function (
    response = { field: [], data: [] }
  ) {
    ...
    data.forEach((coordinateDetails) => {
      signedCoordinatesEnergyList.push(
        this.convertToSignedCoordinates(
          coordinateDetails,
          latDirIndex,
          latIndex,
          lonDirIndex,
  ...
  this.filterListFindBrightestStar = function (list) {
    let brightestStars = [];
    [BOSTON, LONDON, NCR, SANFRANCISCO].forEach((location) => {
      this.addBrightestStarForOneLocation(
        brightestStars,
        this.filterCoordinatesInRangeAndMaxEnergyStar(
          list,
          this.fireball(location.latitude, location.longitude)
        )
      );
    });

    return brightestStars.sort((a, b) => b.energy - a.energy)[0];
  };
  ...
  this.fireball = function (lat, lon) {
    ...
    const latRange =
      lat > 0 ? { min: minLat, max: maxLat } : { min: -minLat, max: maxLat };
    const lonRange =
      lon > 0 ? { min: minLon, max: maxLon } : { min: -minLon, max: -maxLon };
    return { latRange, lonRange };
  };

  this.convertToSignedCoordinates = function (
    coordinateDetails,
    latDirIndex,
    latIndex,
    lonDirIndex,
  ...
  this.filterCoordinatesInRangeAndMaxEnergyStar = function (list, latlonRange) {
    ...
    list.map((locationAndEnergy) => {
      if (
        locationAndEnergy.lat >= latlonRange.latRange.min &&
        locationAndEnergy.lat <= latlonRange.latRange.max &&
        locationAndEnergy.lon >= latlonRange.lonRange.min &&
        locationAndEnergy.lon <= latlonRange.lonRange.max
      )
        if (!locationAndEnergy.lat || !locationAndEnergy.lon) {
          console.log(locationAndEnergy, "---8-----8------");
        }
      if (locationAndEnergy && locationAndEnergy.lat && locationAndEnergy.lon) {
        inRangeCoordinatesList.push(locationAndEnergy);
      }
    });

    if (inRangeCoordinatesList.length)
      brightestStarForOneCoordinate = inRangeCoordinatesList
